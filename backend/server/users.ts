...
import { users, checkins } from "../drizzle/schema";
...
export const usersRouter = router({
    ...
    try {
      const data = await db
        .select({
          ...
          actionsCount: count(checkins.id),
        })
        .from(users)
  ...
  updateRole: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // 3. Impede remover o último admin
      if (foundUser.role === "admin" && input.role !== "admin") {
        const admins = await db
          .select({ count: count() })
          .from(users)
          .where(eq(users.role, "admin"));

        const totalAdmins = Number(admins[0]?.count ?? 0);
  ...
  timeline: publicProcedure.query(async () => {
    ...
    try {
      ...
      return data.map((item) => ({
        ...
        severity: item.actorRole === "admin" ? "success" : "info",
      }));
    } catch (error) {
      console.error("[users.timeline]", error);
      return [];
    }
  }),
});
